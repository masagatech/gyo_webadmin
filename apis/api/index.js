...
var apiRouter = function(app) {
    ...
    var _is_login = function(req, res, next) {
        ...
        if (flag != 'web' && !login_id) {
            _status = 0;
            _message = 'err_req_login_id';
        }
        if (flag != 'web' && _status && !v_token) {
            _status = 0;
        ...
        if (force_close == 1) {
            ...
            if (!_status) {
                ...
                dclass._query(_q, function(status, data) {
                    ...
                    } else if (flag != 'web' && !data.length) {
                        gnrl._api_response(res, 0, 'err_msg_no_account', {});
                    } else if (flag != 'web' && gnrl._isNull(data[0].v_token)) {
                        gnrl._api_response(res, 0, 'err_msg_not_logged_in', {});
                    } else if (flag != 'web' && v_token != data[0].v_token && v_token != data[0].v_imei_number) {
