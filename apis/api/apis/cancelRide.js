...
var currentApi = function( req, res, next ){
	...
	var cancel_reason_id = gnrl._is_undf( params.cancel_reason_id ).trim();
	var cancel_reason_text = gnrl._is_undf( params.cancel_reason_text ).trim();
		...
		*/
		...
		async.series([
			...
			function( callback ){
				...
				if( _data.role == 'user' ){
					if( _data.ride.l_data.charges.cancel_charge_user ){
						_data.deductAmount = _data.ride.l_data.charges.cancel_charge_user;
					}
				}
				else{
					if( _data.ride.l_data.charges.cancel_charge_driver ){
						_data.deductAmount = _data.ride.l_data.charges.cancel_charge_driver;
					}
				}
				...
				if( _data.deductAmount <= 0 ){
					...
					if( _data.role == 'user' ){
						...
						async.series([
							...
							// Get Wallet
							function( callback ){
								Wallet.get( _data.user.id, 'user', function( status, _wallet ){
						...
						async.series([
							...
							// Get Wallet
							function( callback ){
								Wallet.get( _data.driver.id, 'driver', function( status, _wallet ){
							...
							// Refresh Wallet
							function( callback ){
								Wallet.refreshDriverWallet( _data.driver.id, function( status, data ){ 
									callback( null );
								});
							},
