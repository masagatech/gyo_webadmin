...
//error_reporting( E_ALL );
    ...
    if( isset( $_REQUEST['submit_btn'] ) && $_REQUEST['submit_btn'] == 'Submit' ){
        ...
        $name_exist = $dclass->select( '*', $table, " AND v_type = '".$v_type."'" );

        if( count( $name_exist ) && $name_exist != '' ){
            
            $gnrl->redirectTo($page.".php?succ=0&script=add&msg=keyexists");
        }
        else{
    ...
    if(isset($_REQUEST['a']) && $_REQUEST['a']==2) {
        ...
        if(isset($_REQUEST['id']) && $_REQUEST['id']!="") {
            ...
            if( isset( $_REQUEST['submit_btn'] ) && $_REQUEST['submit_btn'] == 'Update' ) {
                ...
                $name_exist = $dclass->select('*',$table," AND v_type = '".$v_type."' AND id != ".$id."");

                if( count( $name_exist ) && $name_exist != '' ){
                                                // succ=1&msg=edit&a=2&script=edit&id='.$_REQUEST['id']
                    $gnrl->redirectTo($page.".php?succ=1&a=2&script=edit&msg=keyexists&id=".$_REQUEST['id']);
                }
                else{
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <div class="content">
                                            ...
                                            <div class="form-group">
                                                <label>Key # It will not change after Payment method Implemented</label>
                                                <input type="text" class="form-control" id="v_type" name="v_type" value="<?php echo $v_type; ?>" />
                                            </div>
											<div class="form-group">
												<label>Icon [60*60]</label>
												...
												<input type="hidden" name="old_image[v_image]" value="<?php echo $v_image;?>" />
												<?php } ?>
											</div>
											<div class="form-group">
                                                <label>Payment Mode</label>
                                                <select class="select2" name="v_mode" id="v_mode">
                                                    <?php 
													echo $gnrl->get_keyval_drop( array(
														'sandbox' => 'Sandbox',
														'live' => 'Live',
													),$v_mode); ?>
                                                </select>
